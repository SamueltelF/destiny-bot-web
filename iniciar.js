...
conn.sendMessage(mdata_2.id, {image: {url:`http://aleatory-api.xyz:8080/welcome?titulo=BEM%20VINDO(A)&nome=${ale2.participants[0].split('@')[0]}&perfil=${shortpc.data}&fundo=${fundo1}&grupo=BEM VINDO AO GRUPO ${encodeURI(mdata_2.subject)}`},
...
conn.sendMessage(mdata_2.id, {image: {url:`http://aleatory-api.xyz:8080/welcome?titulo=Adeus&nome=${ale2.participants[0].split('@')[0]}&perfil=${shortpc.data}&fundo=${fundo2}&grupo=SAIU DE ${encodeURI(mdata_2.subject)}`}, caption: teks, 
...
case 'connecting':
console.log(colors.green('CONECTANDO.. ALEATORY 3.5'))
break;
...
`〔 - _ ALEATORY MD 3.5 _ - CONECTADO COM SUCESSO.. 〕`))
...
require("./index.js")(conn, qrcode, API_KEY_ALEATORY)
